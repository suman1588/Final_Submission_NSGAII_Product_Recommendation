...
from  MyProblem_recommender3 import MyProblem
...
for ij in res.history:
    ...
    if  ip%10==0:
        ...
        plt.scatter(np.array(lists)[: , 0], np.array(lists)[:,1],np.array(lists)[:,2], s=ip, facecolors=c, edgecolors=c,label=str(ip)+'th Iteration')
